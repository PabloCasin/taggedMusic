...
KO_FOLDER="ko_songs"
URL="https://www.beatport.com/_next/data/EQviIdkYPxv2gxAGmBylh/es/search.json?q="
...
for archivo in "$DIRECTORIO"/*.mp3; do
    ...
    fullURL="${URL}"
    json=$(curl -s "$fullURL")
    data=$(echo "$json" | jq -r ".pageProps.dehydratedState.queries[0].state.data.tracks.data")
